...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    ...
    if (storedCart.length > 0) {
      ...
      if (formattedCart.length === 0) {
        setMessage({
          type: "warning",
          text: "No valid items in cart! Redirecting to products...",
        });
  ...
  const handleOrderConfirmation = async (e) => {
    e.preventDefault();
    let { name, email, country, state, city, address, postalCode, phone } =
      shippingInfo;
    ...
    if (parseFloat(priceToUse) <= 0 && discountRate < 1) {
      setMessage({
        type: "warning",
        text: "Total amount must be greater than $0.00 unless 100% discount is applied.",
      });
      return;
    }
    ...
    try {
      ...
      if (parseFloat(priceToUse) === 0) {
        ...
        if (!paymentRes.success) {
          setMessage({
            type: "danger",
            text: paymentRes.error || "Payment failed.",
          });
          return;
        }
      ...
      if (!orderData.success) {
        setMessage({
          type: "danger",
          text: orderData.message || "Failed to save order.",
        });
        return;
      }
      ...
      setMessage({
        type: "success",
        text:
          parseFloat(priceToUse) === 0
            ? "Order completed for free!"
            : "Redirecting to CoinPayments...",
      });
  ...
  return (
    <Container className="mt-5 mb-5">
      ...
      <Row className="checkout">
        <Col md={6} xs={12}>
          <Card
            className="shadow-sm mb-4 h-100 d-flex flex-column"
            style={{ border: "1px solid #ecf0f1", position: "relative" }}
          >
            <Card.Body
              style={{
                flex: "1 1 auto",
            ...
            >
              <h4 className="mb-4" style={{ color: "#1a3c34" }}>
                Order Summary
              </h4>
              {cartItems.length > 0 ? (
                <Row>
                  {cartItems.map((item) => (
                    <Col md={4} xs={6} key={item.id} className="mb-3 d-flex">
                      <Card
        ...
        >
          ...
          >
            ...
            >
              ...
              />
              <div
                style={{ fontSize: "20px", color: "#7f8c8d", marginTop: "4px" }}
              >
                More below...
              </div>
            </div>
          ...
          >
            <Card.Body>
              <div className="mb-4">
                ...
                >
                  Choose your preferred cryptocurrency and enjoy smooth, secure
                  blockchain payments.
                </p>
              </div>
